...
if ($stmt = $con->prepare('SELECT user_id, password FROM USERS WHERE username = ?')) {
	...
	if ($stmt->num_rows > 0) {
        ...
        if ($stmt = $con->prepare('INSERT INTO USERS (username, password, first_name,last_name) VALUES (?, ?, ?,?)')) {
	    // We do not want to expose passwords in our database, so hash the password and use password_verify when a user logs in.
	    //$password = password_hash($_POST['password'], PASSWORD_DEFAULT);
	    $stmt->bind_param('ssss', $_POST['username'], $_POST['password'], $_POST['first_name'],$_POST['last_name']);
	    $stmt->execute();
